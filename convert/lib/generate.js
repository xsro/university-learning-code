...
async function scan(folder, projectDir) {
  ...
  for (const d of items) {
    ...
    if (s.isDirectory() && !d.startsWith(".")) {
      ...
      s.isFile() &&
      !d.startsWith("_") &&
      (d.endsWith("mlx") || d.endsWith("ipynb"))
    ) {
...
async function generate(
  ...
  let shrink = [];
  let matlab = "";
  let count = formerShrink.reduce(
    (pre, cur) => (pre < cur.num ? cur.num : pre),
    0
  );
  ...
  for (const f of await fs.readdir(projectDir)) {
    ...
    if (s.isDirectory() && /\d.*/.test(f)) {
      content += `\n<h2>${f.substring(0, 1)}<a name="${f.substring(
        1
      )}" href="#${f.substring(1)}">${f.substring(1)}</a></h2>\n`;
      ...
      for (const file of files) {
        ...
        if (ext == ".mlx") {
          ...
          matlab += `${resolve(projectDir, file)}\t${resolve(
            publicDir,
            dst
          )}\n`;
        }
        ...
        if (findItermIdx >= 0) {
          ...
          iterms.push({ fileRelNoExt, links: [{ ext, dst }] });
        }
      }
      const getNum = (val) => {
      ...
      const sortedIterms = iterms.sort(
        (a, b) => getNum(a.fileRelNoExt) - getNum(b.fileRelNoExt)
      );
      const contentText = sortedIterms.reduce((pre, iterm) => {
        const linksText = iterm.links.reduce(
          (prelink, link) =>
            prelink +
            `<a href="${link.dst}">${link.ext.replace(".", "")}</a>\n`,
          ""
        );
  ...
  const shrinkText = shrink
    .sort(compareShrink)
    .map((val) => val.number + "\t" + val.fileRel)
    .join(EOL);
